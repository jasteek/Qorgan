...
app.post('/api/ai/chat', async (req, res) => {
  ...
  if (GEMINI_KEY) {
    try {
      ...
      const langName = language === 'kk' ? 'казахском (KK)' : language === 'en' ? 'английском (EN)' : 'русском (RU)';
      const langInstruction = `ВАЖНО: Ты ОБЯЗАН отвечать ТОЛЬКО на ${langName} языке. Не используй другие языки ни при каких обстоятельствах.`;

      const data = JSON.stringify({
        systemInstruction: {
          parts: [{ text: `Ты — Самрұқ (Samruk), AI-ассистент по безопасности в приложении Qorǵan. Твоя миссия — защищать и поддерживать пользователей. Помогай в экстренных ситуациях, предлагай безопасные маршруты, оказывай психологическую поддержку и советы по безопасности. Давай короткие, лаконичные, дружелюбные и поддерживающие ответы. Будь заботливым и профессиональным.\n\n${langInstruction}` }]
        },
        contents: contents,
        generationConfig: {
          temperature: 0.7,
